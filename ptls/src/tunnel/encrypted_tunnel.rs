    ...
    W: AsyncWriteExt + Unpin,
{
    ...
    pub async fn send(&self, content_type: ContentType, payload: &[u8]) -> Result<(), Error> {
    ...
    pub async fn receive(&self) -> Result<(ContentType, Vec<u8>), Error> {
...
async fn encrypted_tunnel() -> Result<(), super::Error> {
    ...
    server.send(content_type.clone(), &payload).await?;
    assert_eq!(peer.receive().await?, (content_type, payload));
...
async fn encrypted_recursive_tunnel() -> Result<(), super::Error> {
    ...
    tunnel.send(content_type.clone(), &payload).await?;
    assert_eq!(tunnel.receive().await?, (content_type, payload));
